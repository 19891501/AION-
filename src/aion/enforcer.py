...
class Transition:
    ...
    def canonical(self) -> str:
        return json.dumps({"actor": self.actor, "action": self.action, "target": self.target, "params": self.params}, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
    def hash(self) -> str:
...
class Enforcer:
    def __init__(self, secret: bytes, *, ttl_sec: float = 60.0, policy_hash: str = "policy-v0", world: Callable[[Transition], Any] | None = None) -> None:
        self._secret = secret
        self.ttl_sec = ttl_sec
        self.policy_hash = policy_hash
        self._world = world
        self._tokens: dict[str, Authorization] = {}
        self.attempts = self.blocked = self.executed = 0
    ...
    def issue(self, tau: Transition, *, now: float | None = None) -> Authorization:
        ...
        auth = Authorization(tid, th, now, exp, self.policy_hash, self._sign(msg))
        self._tokens[tid] = auth
        return auth
    def execute(self, tau: Transition, auth: Authorization | None, *, now: float | None = None) -> EffectResult:
        self.attempts += 1
        now = time.time() if now is None else now
        if auth is None:
            self.blocked += 1
...
def attack_bench(enforcer: Enforcer, tau: Transition, valid: Authorization) -> dict:
    results = []
    def try_attack(name, t, a):
        r = enforcer.execute(t, a)
        results.append({"attack": name, "allowed": r.allowed, "effect": r.effect, "reason": r.reason})
    try_attack("sans_auth", tau, None)
    try_attack("token_inconnu", tau, Authorization("dead", tau.hash(), 0, 1e12, enforcer.policy_hash, "00"))
    try_attack("signature_invalide", tau, Authorization(valid.token_id, valid.transition_hash, valid.issued_at, valid.expires_at, valid.policy_hash, "ff" * 32))
    try_attack("transition_modifiee", Transition(tau.actor, tau.action, "autre-cible", dict(tau.params)), valid)
    try_attack("nonce_reutilise", tau, valid)
    try_attack("nonce_reutilise_2", tau, valid)
    pure = [x for x in results if x["attack"] != "nonce_reutilise"]
    return {"n_attacks": len(pure), "unauthorized_effects": sum(x["effect"] for x in pure), "detail": results, "pass": sum(x["effect"] for x in pure) == 0}
